...
function resolveReference(
		...
		if (token && typeof token === 'object' && '$value' in token) {
			...
			if (typeof value === 'string' && value.startsWith('{')) {
				...
				if (value === reference) {
					...
					if (coreTokens) {
						const coreResolved = resolveReference(
							reference,
							coreTokens,
							new Set()
						);
	...
	if (parts.length === 3 && parts[0] === 'color') {
		...
		if (
			flattenedToken &&
			typeof flattenedToken === 'object' &&
			'$value' in flattenedToken
		) {
...
function resolveAllReferences(
	...
	if (Array.isArray(obj)) {
		return obj.map((item) =>
			resolveAllReferences(item, resolveFrom, coreTokens, visited)
		);
	}
